...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $existing_admin_query = "SELECT * FROM admin WHERE email='$email' OR mobile='$mobile' LIMIT 1";
    $result = $conn->query($existing_admin_query);
    ...
    // If no errors, insert admin data into database
    if (empty($errors)) {
        // Hash the password using bcrypt
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);

        // Insert admin data into admin table
        $insert_query = "INSERT INTO admin (name, email, mobile, password) VALUES ('$name', '$email', '$mobile', '$hashed_password')";
